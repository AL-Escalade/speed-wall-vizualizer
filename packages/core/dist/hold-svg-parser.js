 ...
 */
function parseSvgDocument(svgContent) {
 ...
 */
function findElementByAttribute(parent, tagName, attrName, attrValue) {
 ...
 */
function findElementByIdOrLabel(doc, tagName, idOrLabel) {
    // Try by id first
 ...
 */
function findElementByIdOrLabelMultiTag(doc, tagNames, idOrLabel) {
 ...
 */
function removeUnwantedAttributes(element, removeId = true) {
    const attrsToRemove = [];
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if (child.nodeType === 1) { // Element node
            removeUnwantedAttributes(child, removeId);
        }
    }
}
/**
 ...
 */
function extractCircleCenter(doc, circleId) {
 ...
 */
function extractAllCircles(doc) {
    const circles = [];
    const elements = doc.getElementsByTagName('circle');
    for (let i = 0; i < elements.length; i++) {
        const circle = elements[i].cloneNode(true);
 ...
 */
function simplifyCompoundPath(d) {
 ...
 */
function extractPathElement(doc, pathId) {
...
function shouldSkipElement(element) {
    ...
    if (id === 'insert')
        return true;
    // Skip prise element
    if (id === 'prise' || label === 'prise')
        return true;
    // Skip label zone text elements
    if (label && label.startsWith('label'))
        return true;
    return false;
}
/**
 ...
 */
function extractAllVisualElements(doc) {
    const elements = [];
 ...
 */
function extractLabelZones(doc) {
    const zones = {};
    const labelMap = {
        'label-up': 'up',
        'label-down': 'down',
 ...
 */
export function parseHoldSvg(svgContent) {
 ...
 */
export async function loadHoldSvg(holdType) {
 ...
 */
export function loadHoldTypesConfig() {
 ...
 */
export function getHoldTypeConfig(holdType) {
 ...
 */
export function getHoldShowArrow(holdType) {
    return getHoldTypeConfig(holdType).showArrow ?? true;
}
/**
 * Clear the hold types configuration cache
 */
export function clearHoldTypesConfigCache() {
    holdTypesConfigCache = null;
}
//# sourceMappingURL=hold-svg-parser.js.map